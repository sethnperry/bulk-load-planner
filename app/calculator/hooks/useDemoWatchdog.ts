...
//
// Backs the shareable demo account: enforces "only one active session,
// newest link-visit always wins" plus an idle-timeout backstop. Entirely a
// no-op for every real user -- gated on authUserId matching the one known
// demo account id, so this has zero effect outside the demo account.
//
...
//     makes "open the link again" == "take over."
...
export function useDemoWatchdog(authUserId: string) {
  ...
  const isDemo = !!demoUserId && authUserId === demoUserId;
  ...
  useEffect(() => {
    ...
    async function endSession(reason: string) {
      ...
      router.replace(`/demo/ended?reason=${reason}`);
    }
