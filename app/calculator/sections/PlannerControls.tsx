...
export default function PlannerControls(props: any) {
  ...
  // Derived values for the compartment modal.
  // (Avoid Turbopack parse issues with ternary + IIFE returning JSX in the render.)
  const compNumber = compModalComp ?? 0;

  const c =
    compModalComp == null
      ? null
      : compartments.find((x: any) => Number(x.comp_number) === compNumber);

  const trueMax = Number(c?.max_gallons ?? 0);
  const headPct = compModalComp == null ? 0 : headspacePctForComp(compNumber);
  const effMax = compModalComp == null ? 0 : effectiveMaxGallonsForComp(compNumber, trueMax);
  const sel = compModalComp == null ? null : compPlan?.[compNumber];
  const isEmpty = compModalComp == null ? false : !!(sel as any)?.empty || !(sel as any)?.productId;
  const planned = compModalComp == null ? 0 : (plannedGallonsByComp?.[compNumber] ?? 0);
...
if (isEmpty) {
  codeColor = "rgba(180,220,255,0.9)";
} else if (typeof prod?.hex_code === "string" && prod.hex_code.trim()) {
  codeColor = prod.hex_code.trim();
}
