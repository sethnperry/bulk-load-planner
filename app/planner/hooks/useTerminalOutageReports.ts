...
import { mostRecentClearingCheckpoint, hhmmInTimeZone } from "../utils/dates";
 ...
 * into one ticker string. RLS on terminal_outage_reports already narrows
 ...
 * Planner.
 */
export function useActiveOutageBanner(terminalId: string | null): { message: string | null } {
  ...
  const fetchAndCompose = useCallback(async () => {
    if (!terminalId) { setMessage(null); return; }
    ...
    if (error || !data || data.length === 0) { setMessage(null); return; }
    ...
    const messages = rows.map((r) => {
      ...
      const hhmm = hhmmInTimeZone(new Date(r.created_at).getTime(), tz);
      const truck = r.truck_label || "?";
      if (r.report_type === "out_of_product") {
        const company3 = (companyNameById.get(r.company_id) ?? "").slice(0, 3) || "???";
        return `${company3} ${truck} - Out of ${productName} @ ${hhmm}hr`;
      }
      const initials = initialsOf(displayNameByUserId.get(r.reporter_user_id) ?? "");
      return `${initials} ${truck} OOA ${productName} @ ${hhmm}hr`;
    });
